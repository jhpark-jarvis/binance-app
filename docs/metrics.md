...
Aggregate Trade는 최근 체결·taker flow 테이블을 위한 실시간 원본 데이터로 유지한다. 장애 시간의
Trade 공백은 허용하므로, 행 수·시간 간격·최근 체결 갱신 여부를 데이터 복구 성공 또는 연속성 지표로
사용하지 않는다. 체결 이벤트의 buyer-maker 플래그는 taker 방향을 파생할 때만 사용하며, 투자 신호로
해석하지 않는다.
...
| Recent trades | Aggregate Trade의 최근 12개 체결 | 실시간 체결 흐름의 관측용 보조 정보다. 장애 구간의 완전성은 보장하지 않는다. |
